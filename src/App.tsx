...
export const App = () => {
    ...
    function howToPlayButtonWasPressed() {
        const currentState = state.instructionalShouldShow
        setState({...state, instructionalShouldShow: !currentState})
    }

    function aboutButtonWasPressed() {
        const currentState = state.instructionalShouldShow
        setState({...state, aboutSectionShouldShow: !currentState})
    }
    ...
    function modalWasClicked() {
        setState(state => ({ ...state, leaderboardShouldShow: false, instructionalShouldShow:false, aboutSectionShouldShow:false }))
    }
    ...
    return (
        <div>
            ...
            {state.instructionalShouldShow &&
                <Instructional modalWasClicked={modalWasClicked}></Instructional>}

            {state.aboutSectionShouldShow &&
                <AboutSection modalWasClicked={modalWasClicked}></AboutSection>}

            <div className='top-bar'>
                <h1>BOOM SWEEPER</h1>
                <button onClick={ () => howToPlayButtonWasPressed()}>HOW TO PLAY</button>
                <button onClick={ () => aboutButtonWasPressed()}>ABOUT</button>
            </div>
